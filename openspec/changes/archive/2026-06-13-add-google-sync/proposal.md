...
- 手動同期ボタンから、同期設定が ON のサービスへ GAS 同期リクエストを発火
- `app.js` に同期設定の読み込み・保存ロジックを追加
- GAS テンプレート/セットアップ手順に `syncCalendar()` / `syncGTasks()` 関数と `doPost` ハンドラを追加

## Impact
- Affected specs: `google-sync` (新規)
- Affected code: `web/index.html`, `web/js/api.js`, `web/js/app.js`, `docs/GOOGLE_SYNC_SETUP.md`, `web/gas-api-template.json`
