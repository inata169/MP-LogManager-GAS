...
> [!IMPORTANT]
> Google 同期は **手動同期** です。タスクを保存しても Google Calendar / Google Tasks へは自動同期されません。
> 設定画面で Calendar / Tasks 同期を ON にしたうえで、Web App ヘッダーの同期ボタンから実行してください。
> Calendar 同期は、期限日があり、未完了で、タスク単位の Calendar 同期が OFF ではないタスクだけを送信します。
...
1. Web App の設定画面で、必要な同期を ON にします。
   - Google Calendar 同期
   - Google Tasks 同期
2. タスクを保存します。
   - この時点では Google 側へ自動同期されません。
   - Tasks / Journal の保存は GAS / Google Drive への保存だけを行います。
3. ヘッダーの同期ボタン（☁️）を押して、Google Calendar / Google Tasks への同期を手動実行します。

Calendar 同期では次のタスクだけが送信されます。

- 未完了
- 期限日がある
- タスク単位の Calendar 同期が OFF ではない

Google Tasks 同期では、未完了タスクが `MP-LogManager` リストへ送信されます。
...
> 4. タスク保存だけでは同期されません。同期ボタン（☁️）を押して手動同期してください。
