...
### ⚠️ [重要] 手動実行による権限の有効化 (認可)

デプロイ後、一度だけ手動で関数を実行して、Google にアプリの権限（Drive へのアクセスなど）を承認させる必要があります。

1.  エディタ画面上部で、実行する関数として **`doGet`** などを選択します。
2.  **[実行]** ボタンをクリックします。
3.  「承認が必要です」というポップアップが出るので、指示に従って **[権限を確認]** ＞ **[アカウント選択]** ＞ **[詳細]** ＞ **[移動]** ＞ **[許可]** と進めてください。
4.  実行ログに完（完了）と出れば、これで Web アプリからの通信が許可されました。
